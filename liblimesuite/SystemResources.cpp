...
std::string lime::locateImageResource(const std::string &name)
{
    ...
    {
        const std::string fullPath(searchPath + "/20.10/" + name);
...
{
    const std::string destDir(lime::getAppDataDirectory() + "/images/20.10");
    const std::string destFile(destDir + "/" + name);
    const std::string sourceUrl("https://downloads.myriadrf.org/project/limesuite/20.10/" + name);
