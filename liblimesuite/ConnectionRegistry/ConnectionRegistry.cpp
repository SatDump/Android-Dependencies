...
IConnection *ConnectionRegistry::makeConnection_fd(const ConnectionHandle &handle, int fd, bool is_fx3)
{
    if(is_fx3)
        return  (new ConnectionFX3Entry())->make_fd(handle, fd);
    else
        return  (new ConnectionFT601Entry())->make_fd(handle, fd);
    return nullptr;
}
