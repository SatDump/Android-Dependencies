...
LMS7_Device* LMS7_Device::CreateDevice_fd(const lime::ConnectionHandle& handle, int fd, bool is_fx3, LMS7_Device *obj)
{
    LMS7_Device* device;
    if (obj)
    {
    ...
    auto conn = lime::ConnectionRegistry::makeConnection_fd(handle, fd, is_fx3);
    if (!conn)
        return nullptr;
