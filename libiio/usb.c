...
static int usb_close_pipe(struct iio_context_pdata *pdata, uint16_t pipe_id)
{
	...
	ret = libusb_control_transfer(pdata->hdl, LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_INTERFACE, IIO_USD_CMD_CLOSE_PIPE,
...
struct iio_context *usb_create_context_fd(unsigned int bus, int fd, uint16_t address, uint16_t intrfc)
{
	...
	libusb_wrap_sys_device(usb_ctx, (intptr_t)fd, &hdl);
	usb_dev = hdl->dev;
	...
	libusb_set_configuration(hdl, 1);
